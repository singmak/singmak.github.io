...
Create `handler.ts` in the project base folder.  

Import `getUsers` from `users.ts`.  

Add an async function `getUsers` to `handler.ts`  

Return `statusCode` and `body` in the API response. Make sure that body is a `string`.

API Gateway will use the returning value to determine the API response status code and body.
```typescript
...
The reason to put the actual logic of the function in `users.ts` intead of directly putting it in `handler.ts` is to make it easier for the original express app to also reuse the same logic in `users.ts`.
...
Open `cdk/lib/cdk-stack.ts` and add the resources to create the Lambda function
```typescript
...
API Gateway API for creating the Rest API to trigger the Lambda function
```typescript
...
Since you didn't remove the Express app in the project, you can still run `npm start` in the project for locally testing the API function.
