...
```mermaid
graph TB
    C(Web Client)--Get-->INV(Inventory Service)
    C--Get-->PRO(Product Catalog Service)

    subgraph INVD[Inventory Database]
        INT[Inventory Table]
    end
    subgraph PROD[Product Database]
        PT[Product Table]
    end

    INV--Read/Write-->INVD["fa:fa-database" Invenotory Database]
    PRO--Read/Write-->PROD["fa:fa-database" Product Database]
```
...
sequenceDiagram
    ...
    participant PRO as Product Catalog Service
    C->>PRO: GET /products?manufacturer=[Selected Manufactorer]
...
```mermaid
graph TB
    C(Frontend)--Get-->BFF(BFF service)
    BFF--Get-->INV(Inventory Service)
    BFF--Get-->PRO(Product Catalog Service)
    ...
    INV--Read/Write-->INVD
    PRO--Read/Write-->PROD
    BFF--Read/Write-->CACHE["fa:fa-database" Redis Cache]
```
