...
Directly talking with the traditional SQL database in a AWS Lambda function can be trickly because of the following reasons:
1. Different Lambda function requests are completely separated, they cannot share database connections, so if there're 100 Lambda requests, there will be 100 DB connections.
2. Even if you try to keep the datbase connection around and share it with other Lambda requests, it won't work because the DB connection can be killed when the Lambda function is in a "sleep state", and when the Lambda function is waked up again, the Lambda function would try to restore to the previous state. It still thinks it's connected to the database but actually it's not. I had a lot of issues with this with the pg library. The connected state variable is still true even the connection was lost already which cause the pg library unable to recover the database connection. My team and me were having so much trouble with this and we decided to just screw it and just use a new database connection in every request and make our life easier.
3. AWS Lambda functions can scale automatically but the database is not. If there are a lot Lambda requests to the same database at the same time, the number of connections to the database may exceed the allowed limit and refusing new connections to the database. so far we didn't face this problem after we decided that we disconnected from the database when a request is done. However, if there are really a large amount of concurrent Lambda executions, it could still exceed the maximum DB connections limit.
To deal with different kinds of issues in connecting with database, now every Lambda request would createa new connection to the database and disconnect from the database when it's done. It's as inefficient as it sounds. While not much.. the overhead of creating a new connection in every Lambda requests make the API performance worse.

What if the Lambda doesn't need to connect to the database directly and let Hasura does it? and what are the benefits of doing so?
If a Lambda function call Hasura to get the data from the database, there would be no need to deal with those connection problems anymore and there will be no overhead of connecting to the database in every request. We won't need to worry about exceeding the maximum number of database connections because Hasura keeps a constant amount of the connection to the database. Lambda functions can just query the data from Hasura via simple Graphql API Https requests.
