...
Hasura Actions is a way for us to extend the auto generate Graphql API. For example, if you want to do a customized input validation in inserting data into database, you can create a webhook API to do the data validation and then do the insert. Be default, the input validation is only based on the DB schema.
...
I think Hasura is great as a replacement of the usual CRUD Rest API that we build to talk with the database. Frontend can freely query the data it needs with the Graphql API and we can configure the role permissions in accessing the data.
As mentioned, while Hasura can generate the graphql API for us, it's still our job to immplement the authentication and authorization probably. The freedom of querying the data also means that we need to be careful about not making a query that performs poorly and overload the server. The (official doc)[https://hasura.io/learn/graphql/hasura-advanced/security/6-timeouts/] also suggests that we should set a timeout in the database so that the database can be protected from the attack of overloading the server by requests that take a long time to load.