...
1. It's very hard if not impossible to share database connections between Lambda requests. If you try to keep the database connection around and share it with other Lambda requests, it won't work because the DB connection can be killed when the Lambda function is in a "sleep state", and when the Lambda function is waked up again, the Lambda function would try to restore to the previous state. It still thinks it's connected to the database but actually, it's not. I had a lot of issues with this with the pg library. The connected state variable is still true even the connection was lost already which cause the pg library unable to recover the database connection. My team and I were having so much trouble with this and we decided to just screw it and just use a new database connection in every request and make our life easier.
...
To deal with different kinds of issues in connecting with the database, we decided that every Lambda request just create a new connection to the database and disconnect from the database just between the execution is done. It's as inefficient as it sounds. While not much... the overhead of creating a new connection in every Lambda request makes the API performance worse.

What if the Lambda function doesn't need to connect to the database directly and let Hasura does it? and what are the benefits of doing so?
If a Lambda function calls Hasura to get the data from the database, there would be no need to deal with those connection problems anymore and there will be no overhead of connecting to the database in every request. We won't need to worry about exceeding the maximum number of database connections because Hasura keeps a constant amount of the connection to the database. Lambda functions can just query the data from Hasura via simple Graphql API HTTPS requests.
